...
Generates track meshes and props only (textures generated separately)

Run with: blender --background --python tracks.py
"""
...
try:
    import bpy
    from lib.track_geometry import TrackGeometry
    from lib.prop_geometry import PropGeometry
except ImportError as e:
    print(f"Error: {e}")
    print("Must run from Blender with generators module available")
    print("Usage: blender --background --python tracks.py")
    sys.exit(1)
...
def generate_track_segment(segment_type):
    ...
    # Export mesh
    tri_count = export_obj(obj, f"track_{segment_type}.glb")
...
def generate_prop(prop_type):
    ...
    elif prop_type == 'crystal_formation':
    ...
    filename = f"{prop_type}.glb" if prop_type == 'crystal_formation' else f"prop_{prop_type}.glb"
    tri_count = export_obj(obj, filename)
