...
Copied from: nethercore-ai-plugins/zx-procgen/skills/procedural-sounds/references/sound_parser.py
...
def add_reverb(
    ...
    # Apply multiple delay lines for richer reverb
    delays = [delay_samples, int(delay_samples * 1.3), int(delay_samples * 1.7)]
    decays = [decay, decay * 0.8, decay * 0.6]

    for d, dec in zip(delays, decays):
        if d < len(signal):
            reverb = np.zeros_like(signal)
            for i in range(d, len(signal)):
                reverb[i] = signal[i - d] * dec + reverb[i - d] * dec * 0.5
            output = output * (1 - mix) + reverb * mix

    return output.astype(np.float32)


def apply_effects(signal: np.ndarray, effects: list) -> np.ndarray:
...
def generate_sfx(spec: Dict[str, Any]) -> np.ndarray:
    ...
    # Apply effects chain
    if 'effects' in sound:
        mixed = apply_effects(mixed, sound['effects'])
